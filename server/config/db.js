...
async function initDatabase() {
  try {
    ...
    if (env.NODE_ENV === 'production') {
      console.error(`❌ PostgreSQL initialization failed in production: ${err.message}`);
      throw err;
    }
  ...
  for (const c of DEMO_CERTIFICATES) {
    ...
    if (app && instr && owner) {
      memoryStore.certificates.push({
        ...
        pdf_url: c.pdf_url,
        qr_token: c.qr_token,
        qr_url: `${env.PUBLIC_APP_URL}/verify/${c.certificate_number}`,
...
async function query(text, params = []) {
  ...
  if (t.startsWith('insert into certificates')) {
    ...
    const nc = {
      ...
      qr_url: `${env.PUBLIC_APP_URL}/verify/${certificate_number}`,
      issued_by: 'Legal Metrology Department (Govt. of India)',
      officer_name: app?.assigned_to_name || 'Rajesh Sharma (LMO Officer)',
      created_at: new Date()
    };
