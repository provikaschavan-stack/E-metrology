...
async function testApplications() {
  ...
  if (updateRes.status !== 200 || updateData.data?.application?.status !== 'FIELD_VERIFICATION') {
    throw new Error(`Expected FIELD_VERIFICATION transition: ${JSON.stringify(updateData)}`);
  }
  console.log('✅ 8. LMO: PUT /api/applications/:id -> Status:', updateRes.status, 'Updated App Status:', updateData.data?.application?.status);

  // 8b. LMO submits the inspection record, transitioning the application to UNDER_REVIEW
  const verificationRes = await fetch(`${BASE_URL}/verifications`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${lmoAuth.token}`
    ...
    body: JSON.stringify({
      ...
      accuracy_result: 'PASS - Within statutory tolerance',
      seal_condition: 'PASS - Official seal intact',
      document_result: 'PASS - Model approval verified',
      result: 'PASS'
    })
  });
  const verificationData = await verificationRes.json();
  if (verificationRes.status !== 201 || verificationData.data?.application_status !== 'UNDER_REVIEW') {
    throw new Error(`Expected UNDER_REVIEW transition: ${JSON.stringify(verificationData)}`);
  }
  console.log('✅ 8b. LMO: POST /api/verifications -> Status:', verificationRes.status, 'Updated App Status:', verificationData.data?.application_status);
  ...
  if (approveRes.status !== 200 || approveData.data?.application?.status !== 'APPROVED') {
    throw new Error(`Expected APPROVED transition: ${JSON.stringify(approveData)}`);
  }
  ...
  if (trackRes.status !== 200 || trackData.data?.application?.status !== 'APPROVED' || trackData.data?.application?.applicant_email) {
    throw new Error(`Expected sanitized public tracking response: ${JSON.stringify(trackData)}`);
  }
  console.log('✅ 10. Public Tracking: GET /api/applications/track/:appNumber -> Status:', trackRes.status, 'Tracked Status:', trackData.data?.application?.status, 'PII: sanitized');
