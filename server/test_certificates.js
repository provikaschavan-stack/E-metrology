...
async function runTests() {
  ...
  const detailedAdminLogin = await request({
    hostname: 'localhost',
    port: 5000,
    path: '/api/auth/login',
    method: 'POST',
  ...
  const detailedCert = await request({
    ...
    method: 'GET',
    headers: { Authorization: `Bearer ${detailedAdminLogin.data.data.token}` }
  });
  ...
  console.log('✅ 6b. Application approved. Status:', approveRes.data.data?.application?.status);
