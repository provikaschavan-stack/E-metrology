...
async function testInstruments() {
  ...
  if (updateRes.status !== 200 || updateData.data?.instrument?.status !== 'PENDING_VERIFICATION') {
    throw new Error(`Business instrument edits must preserve workflow status: ${JSON.stringify(updateData)}`);
  }
  console.log('6. PUT /api/instruments/:id:', updateRes.status, 'Capacity updated; status remains:', updateData.data?.instrument?.status);
